...
class DiceLossNoFNS(nn.Module):

    def __init__(self, alpha=0.35, beta=0.65, size_average=True, reduce=True):
        super(DiceLossNoFNS, self).__init__()
        self.alpha = alpha
        self.beta = beta
    ...
    def forward(self, preds, targets, weight=False):
        ...
        smooth = smooth.to(preds.device)
        #self.alpha = FP.sum(dim=(0)) / ((FP.sum(dim=(0)) + FN.sum(dim=(0))) + smooth) # anissa: removed these to fix set values
...
class SDiceLoss(nn.Module):
    ...
    def forward(self, preds, targets,weight_map=None):
        ...
        #n = torch.unique(preds)
        # print('unqieu elements preds')
        # print(n)

        # m = torch.unique(ground)
        # print('unqieu elements ground')
        # print(m)
...
def dce_evidence_u_loss(p, alpha, c, current_step, lamda_step, total_step, eps, disentangle, evidence, backbone_pred):
    # c: class number
    criterion_dl = DiceLossNoFNS() # anissa 
    ...
    if alpha.ndim == 5:
        ...
        soft_p = p
        #print('for dce evidence u loss, alpha has 5 dims, did not unsqueeze')
...
def dce_evidence_loss(p, alpha, c, current_step, lamda_step, total_step,eps,disentangle,evidence,backbone_pred):
    ...
    if alpha.ndim == 5:
        # anissa changes:
        soft_p = p
        #print('alpha has 5 dimensions, did not unsqueeze')
        #soft_p = p.unsqueeze(1)
        # soft_p = p
    else:
        soft_p = p

    # print('soft_p shape adn unqiue elemnets')
    # print(soft_p.shape)
    # m = torch.unique(soft_p)
    # print(m)
    ...
    # print('c')
    # print(c)
    # print('soft_p shape and unique elements after get_soft_label')
    # print(soft_p.shape)
    # m = torch.unique(soft_p)
    # print(m)
