...
class TMSU(nn.Module):
    ...
    def __init__(self, args):
        ...
        if dataset=='autopet' or dataset=='autopet2':
            if model_name == 'AU':
                self.backbone = AttUnet(in_channels=2, base_channels=16, num_classes=2)
            else:
                self.backbone = Unet(in_channels=2, base_channels=16, num_classes=2) # changed num of channels and classes
        elif dataset == 'BraTS':
    ...
    def forward(self, X, y, global_step, mode,dataset):
        ...
        if mode == 'train':
            ...
            if dataset == 'BraTS': # or dataset=='autopet':
        ...
        if mode == 'train':
            ...
            print('dce evidence loss before mean')
            print(loss)
            loss = torch.mean(loss)
            return evidence, loss

        elif mode == 'val': # added loss for val - anissa
            loss = dce_evidence_loss(y.to(torch.int64), alpha, self.classes, global_step, self.lambda_epochs,self.total_epochs,self.eps,self.disentangle,evidence,backbone_pred)
            loss = torch.mean(loss)
            return evidence,loss
        else:
